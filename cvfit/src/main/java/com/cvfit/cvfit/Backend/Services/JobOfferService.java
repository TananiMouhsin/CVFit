...
public class JobOfferService {
    ...
    public boolean deleteJobOfferByTitleAndCv(User user, String cvTitle, String jobTitle) {
        Optional<CV> cvOpt = cvRepository.findByUserAndCvNameIgnoreCase(user, cvTitle);
        System.out.println("service");
        if (cvOpt.isEmpty()) return false;
        System.out.println("cv found : "+cvOpt);
        CV cv = cvOpt.get();

        Optional<JobOffer> jobOfferOpt = jobOfferRepository.findByTitleIgnoreCaseAndCv(jobTitle, cv);
        
        if (jobOfferOpt.isEmpty()) return false;
        System.out.println("joboffer found : "+jobOfferOpt);
        jobOfferRepository.delete(jobOfferOpt.get());
        return true;
    }
}